...
import { mathGameArenaTopics } from "@/data/menuNavigasi";

type Kelas = keyof typeof mathGameArenaTopics;
type Topic = typeof mathGameArenaTopics[Kelas][number];
    ...
    <div className="relative min-h-screen flex flex-col items-center justify-center gradient-space overflow-hidden">
      ...
      <div className="relative z-10 max-w-3xl w-full px-4 py-10">
        ...
        <p className="text-white/60 text-sm text-center mb-8 font-body">
          {!selectedKelas 
            ? "Pilih tingkatan kelas untuk memulai tantangan" 
            : !selectedTopic 
              ? `Arena ${selectedKelas} — Pilih topik` 
        ...
        {!selectedKelas && (
          <div className="animate-slide-up grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-lg mx-auto">
            {(Object.keys(mathGameArenaTopics) as Kelas[]).map((kelas, i) => (
              <button
                key={kelas}
              ...
              >
                ...
                <p className="text-xs text-muted-foreground mt-1 font-body">
                  {mathGameArenaTopics[kelas].length} topik tantangan
                </p>
              </button>
            ))}
          </div>
        ...
        {selectedKelas && !selectedTopic && (
          <div className="animate-slide-up flex flex-col gap-3">
            ...
            <div className="grid grid-cols-1 gap-2">
              {mathGameArenaTopics[selectedKelas].map((topic, i) => (
                <button
                  key={topic.label}
                ...
                >
                  ...
                  <span className="ml-auto text-xs text-accent font-display">PILIH SUBTOPIK →</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="animate-slide-up flex flex-col gap-3">
            ...
            <div className="grid grid-cols-1 gap-2">
              {selectedTopic.subtopics.map((sub, i) => (
                ...
                >
                  <Gamepad2 className="w-5 h-5 text-accent shrink-0 group-hover:scale-110 transition-transform" />
                  <span className="font-body text-sm text-white">{sub.label}</span>
                  <span className="ml-auto text-xs text-accent font-display">🎮 MULAI GAME</span>
                </button>
              ))}
            </div>
          </div>
