...
import { LatexInline } from "@/components/LatexDisplay";
...
const QuizGamezContentPage = () => {
  ...
  const quizData = quizGamezData[key];
  ...
  const setupMeteors = useCallback((qIndex: number) => {
    if (!quizData) return;
    const q = quizData.soal[qIndex];
    const count = q.options.length;
    const spacing = 80 / (count + 1);
    
    // We assume the correct answer is always at index 0 in the options array for simplicity in placeholder data
    // or we can detect it if it starts with "A." or similar if we follow a convention.
    // Looking at quizGamezData, it's just placeholder.
    // Let's assume index 0 is correct for now or randomize it.
    ...
    setMeteors(
      q.options.map((opt, i) => ({
        ...
        label: opt,
        hit: false,
        correct: i === 0, // Placeholder convention: first option is correct
      }))
    );
    setShipX(50);
    setLaser(null);
  ...
  const handleMeteorClick = useCallback(
    (meteor: MeteorState) => {
      if (locked || meteor.hit || !quizData) return;
      setLocked(true);
      setShipX(meteor.x);
      playLaser();
        ...
        const step = () => {
          ...
          if (p < 1) {
            ...
            if (meteor.correct) {
              ...
              const correctAnswer = quizData.soal[currentQ].options[0];
              setFeedback({ type: "wrong", answer: correctAnswer });
            }

            setTimeout(() => {
              setLaser(null);
              if (currentQ + 1 < quizData.soal.length) {
                setCurrentQ((q) => {
                  const next = q + 1;
                  setupMeteors(next);
                  return next;
    ...
    [locked, currentQ, playLaser, playExplosion, playCorrect, setupMeteors, stopBgMusic, quizData]
  );
      ...
      <div className="relative min-h-screen flex flex-col items-center justify-center gradient-space overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl w-full px-4 py-10 text-center">
          ...
          <p className="text-white/60 font-body text-sm mb-8">
            Tantangan untuk topik "{subtopic}" sedang disiapkan. Silakan coba topik lainnya.
          </p>
          <button
            onClick={() => { navigate(-1); }}
    ...
    return (
      <div className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center animate-slide-up">
          <h1 className="font-display text-4xl md:text-5xl font-black gradient-neon bg-clip-text text-transparent mb-2">ARENA GAME</h1>
          <h2 className="font-display text-2xl md:text-3xl font-bold text-accent text-glow-accent mb-6 uppercase">{quizData.title}</h2>
          ...
          <div className="mt-6">
            <button onClick={() => navigate(-1)} className="text-sm text-muted-foreground hover:text-primary transition-colors cursor-pointer">← Kembali</button>
          </div>
        </div>
      </div>
    );
  ...
  if (finished) {
    const maxScore = quizData.soal.length * 10;
      ...
      <div className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center animate-slide-up">
          ...
          <div className="flex gap-4 justify-center">
            ...
            <button onClick={() => navigate("/math-game-arena")} className="font-display text-sm px-6 py-2 rounded-lg bg-muted text-foreground cursor-pointer hover:opacity-90">Arena</button>
            <button onClick={() => navigate("/menu")} className="font-display text-sm px-6 py-2 rounded-lg bg-muted text-foreground cursor-pointer hover:opacity-90">Menu Utama</button>
          </div>
        </div>
      </div>
    );
  ...
  const q = quizData.soal[currentQ];
    ...
    <div className="relative min-h-screen overflow-hidden select-none">
      ...
      <div className="absolute top-0 left-0 right-0 z-20 flex flex-col items-center px-4 py-3">
        <div className="font-display text-xs text-muted-foreground mb-1">
          SOAL {currentQ + 1}/{quizData.soal.length}
        </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 z-20 px-4 pb-4">
        <div className="bg-card/90 backdrop-blur border border-primary/30 rounded-xl px-6 py-3 max-w-2xl mx-auto box-glow-cyan text-center">
          <div className="font-body text-sm md:text-base text-foreground">
            <LatexInline math={q.soal} />
          </div>
        </div>
      </div>
        ...
        {meteors.map((m) => (
          <button key={m.id} onClick={() => handleMeteorClick(m)} disabled={m.hit || locked}
            ...
            <div className="relative">
              ...
              <div className="absolute inset-0 flex items-center justify-center font-display text-[9px] md:text-[10px] font-bold text-foreground drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)] px-1 text-center leading-tight">
                <LatexInline math={m.label} />
              </div>
            </div>
            {m.hit && (
